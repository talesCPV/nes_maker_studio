// ==========================================
// MÓDULO CONFIGURAÇÕES • dados do projeto e gameplay
...
const CONFIG = (() => {
      ...
      <div style="display:flex;flex-direction:column;height:100%;background:#1e1e1e;overflow:auto">
        <div style="padding:16px 20px;background:#252526;border-bottom:1px solid #333;display:flex;align-items:center;gap:12px">
          ...
          <div style="margin-left:auto;display:flex;gap:8px">
            ...
            <button class="btn-tool" onclick="CONFIG.exportSummary()">📄 Resumo</button>
          </div>
        </div>
        ...
        <div style="display:flex;flex:1;gap:0;overflow:hidden;min-height:0">
          ...
          <div style="width:380px;min-width:380px;background:#181818;border-right:1px solid #333;padding:16px;display:flex;flex-direction:column;gap:14px;overflow:auto">
            <div style="background:#111;border:1px solid #333;border-radius:8px;padding:12px">
              ...
              <div style="margin:10px 0;padding:10px;background:#111;border:1px solid #333;border-radius:6px">
                ...
                <div style="font-size:9px;color:#666;margin-bottom:6px;line-height:1.4">
                  Mesma ideia da força de pulo, mas pra velocidade de movimento (0-255) - o valor é
                  literalmente pixels por frame, sem casas decimais (1 = o mais lento possível, acima
                  disso multiplica). Clique no nome/valor pra editar, ou use ▲▼ pra reordenar a lista -
                  a ordem aqui é só organizacional (o motor usa o ID, não a posição), então dá pra
                  renomear um nível existente e encaixar níveis mais lentos antes dele sem quebrar
                  nenhuma regra que já usa esse nível.
                </div>
          ...
          <div style="flex:1;background:#1e1e1e;padding:16px;overflow:auto;display:flex;flex-direction:column;gap:12px">
            <div style="display:flex;align-items:center;gap:12px">
              ...
              <button class="btn-tool" onclick="CONFIG.addPhase()" style="margin-left:auto;background:#27ae60;color:#fff;padding:6px 14px">+ Nova Fase</button>
            </div>
          ...
          <div style="width:340px;min-width:340px;background:#181818;border-left:1px solid #333;padding:16px;overflow:auto;display:flex;flex-direction:column;gap:12px">
            ...
            <div style="margin-top:8px;background:#111;border:1px solid #333;border-radius:6px;padding:10px">
              ...
              <div style="font-size:10px;color:#666;line-height:1.5">
                A fase escolhe <b style="color:#ffcc00">página de sprites</b> + <b style="color:#8585ff">página de backgrounds</b> e passa a ter acesso a <b>todas as telas</b> desenhadas nesse banco.<br><br>
                ...
                <b style="color:#ff6666">Atenção:</b> com <b>CNROM</b> cada fase escolhe páginas de sprites e backgrounds; com <b>NROM</b> o CHR é sempre págs 0+1.
              </div>
            </div>
            ...
            <div style="background:#111;border:1px solid #333;border-radius:6px;padding:10px">
              ...
              <div style="font-size:10px;color:#666;line-height:1.5">
                ...
                <b style="color:#4ec9b0">Down ↓:</b> platformer clássico.
              </div>
            </div>
          </div>
        </div>
  ...
  function renderSpeedLevels(){
    ...
    el.innerHTML = list.map((f,i) => `
      <div style="display:flex;gap:6px;align-items:center;background:#000;border:1px solid #333;border-radius:4px;padding:4px 6px">
        <div style="display:flex;flex-direction:column;gap:1px">
          <button class="btn-tool" onclick="CONFIG.moveSpeedLevel('${f.id}',-1)" ${i===0?'disabled':''} style="padding:0 4px;font-size:8px;line-height:1.4">▲</button>
          <button class="btn-tool" onclick="CONFIG.moveSpeedLevel('${f.id}',1)" ${i===list.length-1?'disabled':''} style="padding:0 4px;font-size:8px;line-height:1.4">▼</button>
        </div>
        <input value="${esc(f.name)}" oninput="CONFIG.updateSpeedLevelProp('${f.id}','name',this.value)" style="flex:1;background:#000;color:#fff;border:1px solid #444;border-radius:4px;padding:4px;font-size:11px;min-width:0">
        <input type="number" min="0" max="255" value="${f.value}" onchange="CONFIG.updateSpeedLevelProp('${f.id}','value',this.value)" style="width:56px;background:#000;color:#4ec9b0;border:1px solid #444;border-radius:4px;padding:4px;font-size:10px;font-family:monospace" title="px/frame">
        <button class="btn-tool" onclick="CONFIG.deleteSpeedLevel('${f.id}')" style="background:#7d2525;color:#fff;font-size:9px;padding:2px 5px">🗑</button>
      </div>`).join('') || '<div style="color:#666;font-size:10px">Nenhum nível ainda.</div>';
  }
  ...
  function updateSpeedLevelProp(id, prop, val){
    const f = (Project.data?.speedLevels||[]).find(x=>x.id===id); if(!f) return;
    if(prop==='name') f.name = val;
    else { let v = parseInt(val); if(isNaN(v)) v = 0; f.value = Math.max(0, Math.min(255, v)); }
    renderSpeedLevels();
  }
  function moveSpeedLevel(id, dir){
    const list = Project.data?.speedLevels||[]; const i = list.findIndex(x=>x.id===id);
    const j = i+dir; if(i<0||j<0||j>=list.length) return;
    [list[i], list[j]] = [list[j], list[i]];
    renderSpeedLevels();
  }
  ...
  function updateStats(){
    ...
    const pages = Math.max(2, Math.ceil(chrLen / 4096));
    el.innerHTML = `CHR: ${(chrLen/1024)}KB (${pages} pág)<br>Metatiles: ${mtCount}<br>Backgrounds: ${bgCount}<br>Splash: ${splashCount}<br>Fases: ${phaseCount}`;
  }

  // Páginas de 4KB no CHR (alinhado ao CHR Editor / chrPages)
  function getPageCount(){
    const chrLen = Project.data?.chr?.length || 8192;
    return Math.max(2, Math.ceil(chrLen / 4096));
  }
  /** @deprecated prefer getPageCount — mantido p/ refs antigas */
  function getAvailableBanks(){
    return Math.max(1, Math.ceil(getPageCount() / 2));
  }
  function getChrPagesMeta(){
    const n = getPageCount();
    let pages = Project.data?.chrPages;
    ...
    while(pages.length < n){
      ...
      pages.push({ name:'Página '+i, role:(i%2===0?'sprite':'background') });
    }
    return pages.slice(0, n).map((e,i)=>({
      name: (e?.name || ('Página '+i)).toString(),
      role: (e?.role === 'background' || e?.role === 'sprite') ? e.role : (i%2===0?'sprite':'background')
    }));
  }
  function migratePhaseChrPages(phase){
    if(!phase) return;
    const n = getPageCount();
    if(phase.sprite_page === undefined || phase.sprite_page === null){
      const b = phase.bank|0;
      phase.sprite_page = Math.min(n-1, b*2);
    }
    if(phase.bg_page === undefined || phase.bg_page === null){
      const b = phase.bank|0;
      phase.bg_page = Math.min(n-1, b*2+1);
    }
    phase.sprite_page = Math.max(0, Math.min(n-1, phase.sprite_page|0));
    phase.bg_page = Math.max(0, Math.min(n-1, phase.bg_page|0));
  ...
  function pageOptionsHtml(selected, roleFilter){
    ...
    pages.forEach((pg, i)=>{
      // Só páginas do tipo pedido (sprite × background)
      if(roleFilter === 'sprite' && pg.role !== 'sprite') return;
      if(roleFilter === 'background' && pg.role !== 'background') return;
      const sel = (selected|0) === i ? 'selected' : '';
      if(sel) hasSelected = true;
      const tag = pg.role === 'background' ? 'BG' : 'SPR';
      html += `<option value="${i}" ${sel}>${i}: ${pg.name} (${tag})</option>`;
    });
    // Seleção antiga com role diferente: mantém visível até o usuário trocar
    if(!hasSelected && selected != null && pages[selected|0]){
      const pg = pages[selected|0];
      const tag = pg.role === 'background' ? 'BG' : 'SPR';
      html = `<option value="${selected|0}" selected>${selected|0}: ${pg.name} (${tag} · revisar)</option>` + html;
    }
    if(!html){
      const label = roleFilter === 'background' ? 'background' : 'sprite';
      html = `<option value="0">— nenhuma página ${label} —</option>`;
    }
    return html;
  }
  ...
  function renderPhases(){
    ...
    phases.forEach((phase, idx)=>{
      ...
      const sp = phase.sprite_page|0, bp = phase.bg_page|0;
      const chrBadge = isCnrom
        ? `<span style="font-size:9px;background:#1a2a1a;color:#7dcea0;padding:2px 6px;border-radius:3px;border:1px solid #2a5a2a" title="Página de sprites">SPR ${sp}</span>
            <span style="font-size:9px;background:#1a1a2e;color:#8585ff;padding:2px 6px;border-radius:3px;border:1px solid #2a2a4a" title="Página de backgrounds">BG ${bp}</span>`
        : `<span style="font-size:9px;background:#222;color:#aaa;padding:2px 6px;border-radius:3px;border:1px solid #444" title="NROM: CHR fixo págs 0+1">NROM</span>`;
  ...
  function selectPhase(idx){
    ...
    const spriteOpts = pageOptionsHtml(phase.sprite_page, 'sprite');
    ...
    detail.innerHTML=`
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        ${projectMapper === 3 ? `
        <!-- CNROM: páginas CHR por fase -->
        <div style="background:#1a1a00;border:1px solid #665500;border-radius:6px;padding:10px">
          <label style="font-size:11px;color:#ffcc00;font-weight:bold;display:block;margin-bottom:8px">CHR DESTA FASE (CNROM)</label>
          <div style="display:flex;flex-direction:column;gap:8px">
            <div>
              <label style="font-size:10px;color:#7dcea0">Página de Sprites → PPU $0000</label>
              <select id="editPhaseSpritePage" style="width:100%;background:#000;color:#7dcea0;border:1px solid #2a5a2a;border-radius:4px;padding:6px;font-size:12px">
                ${spriteOpts}
              </select>
            </div>
            <div>
              <label style="font-size:10px;color:#8585ff">Página de Backgrounds → PPU $1000</label>
              <select id="editPhaseBgPage" style="width:100%;background:#000;color:#8585ff;border:1px solid #2a2a5a;border-radius:4px;padding:6px;font-size:12px">
                ${bgOpts}
              </select>
            </div>
          </div>
          <div style="font-size:9px;color:#888;margin-top:8px;line-height:1.45">
            No build, monta <b style="color:#ffcc00">8KB = SPR + BG</b> para esta fase (banco CNROM).
          </div>
        </div>` : ''}
        ...
        <div style="display:flex;gap:6px;margin-top:6px">
          <button class="btn-tool" onclick="CONFIG.savePhaseDetail()" style="flex:1;background:#27ae60;color:#fff">💾 Salvar Fase</button>
          <button class="btn-tool" onclick="CONFIG.deletePhase(${idx})" style="background:#c0392b;color:#fff">🗑 Deletar</button>
        </div>
      </div>
    `;
  }
