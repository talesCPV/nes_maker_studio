...
const CHR = (() => {
  ...
  /** Metadados por página de 4KB (índice = página). Págs 0/1 fixas no tipo. */
  let chrPages = [
    { name: 'Sprites', role: 'sprite' },
    { name: 'Backgrounds', role: 'background' }
  ];
        ...
        <div style="display:flex;flex:1;overflow:hidden;min-height:0">
          <div style="width:560px;min-width:560px;background:#181818;padding:12px;display:flex;flex-direction:column;gap:8px;overflow:auto;border-right:1px solid #333">
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
              <select id="bankSelect" title="Página CHR (4KB)" style="background:#111;color:#fff;border:1px solid #444;border-radius:4px;padding:4px 8px;font-size:12px;min-width:160px"></select>
              <button class="btn-tool" style="background:#c0392b;color:#fff" onclick="CHR.addPage()" title="Adiciona 1 página (4KB)">+ PÁG</button>
              <button class="btn-tool" style="background:#333;color:#fff" onclick="CHR.removePage()" title="Remove página atual (só pág ≥ 2)">− PÁG</button>
              <input id="chrPageName" type="text" maxlength="32" placeholder="Nome da página" title="Renomear página" style="background:#111;color:#fff;border:1px solid #444;border-radius:4px;padding:4px 8px;font-size:12px;width:140px">
              <select id="chrPageRole" title="Tipo da página" style="background:#111;color:#fff;border:1px solid #444;border-radius:4px;padding:4px 8px;font-size:12px">
              ...
              <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
                ...
                <div style="flex:1;min-width:120px">
                  ...
                  <div style="display:flex;gap:6px;margin-top:10px;align-items:center;flex-wrap:wrap">
                    <div style="display:flex;gap:6px" id="quickColors"></div>
                    <button type="button" class="btn-tool" id="btnColorSwap" onclick="CHR.beginColorSwap()" title="Troca dois índices de cor nos tiles do metatile (não altera a paleta)" style="background:#6c3483;color:#fff;font-size:11px;padding:4px 10px">change</button>
                  </div>
                  <div id="colorSwapHint" style="font-size:10px;color:#666;margin-top:8px;line-height:1.4">Esquerdo desenha · direito pega cor · teclas 1–4</div>
            ...
            <div style="display:flex;flex:1;min-height:0;overflow:hidden">
              <div style="flex:1;padding:12px;overflow:auto;display:flex;flex-direction:column;gap:8px;background:#111">
                <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                  ...
                  <button class="btn-tool" onclick="CHR.nmsImportFromLibrary()" style="background:#16a085;color:#fff" title="Carregar .tile da biblioteca do usuário">📚 Biblioteca</button>
  ...
  function defaultChrPages(n){
    const pages = [];
    for(let i=0;i<n;i++){
      if(i===0) pages.push({ name:'Sprites', role:'sprite' });
      else if(i===1) pages.push({ name:'Backgrounds', role:'background' });
      else pages.push({ name:'Página '+i, role:(i%2===0?'sprite':'background') });
    }
    return pages;
  }
  function ensureChrPages(){
    const total = Math.max(2, Math.ceil(chrBuffer.length/4096));
    if(chrBuffer.length < total*4096){
      const nb = new Uint8Array(total*4096);
      nb.set(chrBuffer);
      chrBuffer = nb;
    }
    if(!Array.isArray(chrPages)) chrPages = [];
    while(chrPages.length < total){
      const i = chrPages.length;
      if(i===0) chrPages.push({ name:'Sprites', role:'sprite' });
      else if(i===1) chrPages.push({ name:'Backgrounds', role:'background' });
  ...
  function isSpriteBank(bankIdx){
    ensureChrPages();
    const e = chrPages[bankIdx];
    if(e && e.role) return e.role === 'sprite';
    return bankIdx%2===0;
  }
  function syncPageMetaUI(){
    ensureChrPages();
    const nameEl = document.getElementById('chrPageName');
    const roleEl = document.getElementById('chrPageRole');
    const e = chrPages[currentBank] || { name:'Página '+currentBank, role:(currentBank%2===0?'sprite':'background') };
  ...
  function updateBankSelect(){
    ...
    if(!sel) return;
    const total = ensureChrPages();
    if(currentBank >= total) currentBank = 0;
    sel.innerHTML = '';
    for(let i=0;i<total;i++){
      const e = chrPages[i];
      const roleLab = e.role==='background' ? 'BG' : 'SPR';
      const o = document.createElement('option');
      o.value = i;
      o.textContent = `${i}: ${e.name} (${roleLab})`;
      sel.appendChild(o);
    }
    ...
    sel.onchange = e=>{
      ...
      // Mantém o mesmo layout local (tile % 256) na página nova
      const remap = (ti)=>{
        if(ti == null || ti < 0) return ti;
        return next * 256 + (ti % 256);
      };
      selectedTiles = selectedTiles.map(remap);
      if(Array.isArray(overlayTiles) && overlayTiles.length){
        overlayTiles = overlayTiles.map(remap);
      }
      currentBank = next;
    ...
    // binds one-shot for name/role
    const nameEl = document.getElementById('chrPageName');
    const roleEl = document.getElementById('chrPageRole');
    if(nameEl && !nameEl._chrBound){
      nameEl._chrBound = true;
      nameEl.addEventListener('change', ()=>{
        ensureChrPages();
        const v = (nameEl.value||'').trim().slice(0,32) || ('Página '+currentBank);
        chrPages[currentBank].name = v;
        if(currentBank===0) chrPages[0].role = 'sprite';
        if(currentBank===1) chrPages[1].role = 'background';
        updateBankSelect();
      });
    }
    if(roleEl && !roleEl._chrBound){
      roleEl._chrBound = true;
      roleEl.addEventListener('change', ()=>{
        ensureChrPages();
        if(currentBank < 2){ syncPageMetaUI(); return; }
        chrPages[currentBank].role = roleEl.value === 'background' ? 'background' : 'sprite';
        updateBankSelect();
        ensurePaletteMatchesBank();
        renderAll();
      });
  ...
  function setColorSwapHint(msg, active){
    const el = document.getElementById('colorSwapHint');
    const btn = document.getElementById('btnColorSwap');
    if(el){
      el.textContent = msg || 'Esquerdo desenha · direito pega cor · teclas 1–4';
      el.style.color = active ? '#f1c40f' : '#666';
    }
    if(btn){
      btn.style.outline = active ? '2px solid #f1c40f' : '';
      btn.textContent = active ? (colorSwap && colorSwap.step === 1 ? 'change… 2' : 'change… 1') : 'change';
    }
  }
  function beginColorSwap(){
    if(colorSwap){
      colorSwap = null;
      setColorSwapHint(null, false);
      if(typeof Project !== 'undefined' && Project.status) Project.status('Troca de cores cancelada');
      return;
    }
    colorSwap = { step: 0, a: null };
    setColorSwapHint('CHANGE: clique no ZOOM na 1ª cor a trocar (Esc ou change cancela)', true);
    if(typeof Project !== 'undefined' && Project.status) Project.status('Troca de cores: 1ª cor no zoom');
  }
  function sampleZoomColorIndex(px, py){
    const tiles = currentTiles();
    ...
    const off = ti * 16;
    if(off + 16 > chrBuffer.length) return null;
    const p0 = chrBuffer[off+ly], p1 = chrBuffer[off+ly+8], sh = 7 - lx;
    return ((p1 >> sh) & 1) << 1 | ((p0 >> sh) & 1);
  }
  function swapColorIndicesInSelection(cA, cB){
    cA &= 3; cB &= 3;
    if(cA === cB) return 0;
    pushUndo();
    const seen = new Set();
    ...
    for(let i = 0; i < tiles.length; i++){
      ...
      if(off + 16 > chrBuffer.length) continue;
      for(let y = 0; y < 8; y++){
        let p0 = chrBuffer[off+y], p1 = chrBuffer[off+y+8];
        for(let x = 0; x < 8; x++){
          const sh = 7 - x;
          const ci = ((p1 >> sh) & 1) << 1 | ((p0 >> sh) & 1);
          let n = ci;
          if(ci === cA) n = cB;
          else if(ci === cB) n = cA;
          else continue;
          if(n & 1) p0 |= (1 << sh); else p0 &= ~(1 << sh);
          if(n & 2) p1 |= (1 << sh); else p1 &= ~(1 << sh);
        }
        chrBuffer[off+y] = p0;
        chrBuffer[off+y+8] = p1;
      }
    }
    renderAll();
    updateLabels();
  }
  function handleColorSwapClick(px, py){
    if(!colorSwap) return false;
    const ci = sampleZoomColorIndex(px, py);
    if(ci == null){
      if(typeof Project !== 'undefined' && Project.status) Project.status('Clique dentro de um tile do metatile');
      return true;
    }
    if(colorSwap.step === 0){
      colorSwap.a = ci;
      colorSwap.step = 1;
      setColorSwapHint('CHANGE: 1ª cor = ' + (ci+1) + ' — clique na 2ª cor (Esc cancela)', true);
      if(typeof Project !== 'undefined' && Project.status) Project.status('1ª cor: ' + (ci+1) + ' — escolha a 2ª');
      return true;
    }
    const a = colorSwap.a|0, b = ci|0;
    colorSwap = null;
    setColorSwapHint(null, false);
    if(a === b){
      if(typeof Project !== 'undefined' && Project.status) Project.status('Cores iguais — nada a trocar');
      return true;
    }
    swapColorIndicesInSelection(a, b);
    if(typeof Project !== 'undefined' && Project.status)
      Project.status('Cores trocadas: ' + (a+1) + ' ↔ ' + (b+1));
    return true;
  }
...
function paintZoomPixel(px, py, colorSlot){
  ...
  function attachEvents(){
    ...
    zoomCanvas?.addEventListener('mousedown', e=>{
      ...
      if(colorSwap && e.button === 0){
        e.preventDefault();
        handleColorSwapClick(px, py);
        return;
      }
    ...
    if(!window._chrColorSwapEsc){
      window._chrColorSwapEsc = true;
      window.addEventListener('keydown', e=>{
        if(e.key === 'Escape' && colorSwap){
          colorSwap = null;
          setColorSwapHint(null, false);
          if(typeof Project !== 'undefined' && Project.status) Project.status('Troca de cores cancelada');
        }
  ...
  function loadSelectedMetatile(){
    ...
    if(bankSel) bankSel.value=String(currentBank);
    try{ syncPageMetaUI(); }catch(e){}
    ensurePaletteMatchesBank();
    updateLayerUI();
    renderAll(); updateLabels();
  }
  ...
  function nmsImportApplyJson(json, displayName, forceTile){
    if(!json || !json.chr){
      alert('Asset inválido (sem CHR). Use .nms ou .tile.');
      return false;
    }
    const isTile = !!(forceTile || json.format === 'tile');
    ...
    nmsImport.name = json.name || displayName || 'import';
    nmsImport.bank = 0;
    nmsImport.picked = [];
    ...
    if(info){
      ...
      const src = isTile ? ' · .tile' : '';
      info.innerHTML = `<b style="color:#fff">${nmsImport.name}</b> · ${nmsImport.metatiles.length} metatile(s)${charInfo}${src}`;
    }
  ...
  async function nmsImportFromLibrary(){
    if(typeof AssetLibrary === 'undefined'){
      alert('Biblioteca indisponível.\nConfirme se js/library.js está carregado na página.');
      return;
    }
    try{
      const items = await AssetLibrary.list('tile');
      if(!items || !items.length){
        alert('Nenhum .tile na biblioteca.\nSalve metatiles pelo módulo de Personagens (Save to lib).');
        return;
      }
      const lines = items.map((it, i) => {
        const mt = it.metatile_count != null ? it.metatile_count + ' mt' : '';
        return (i+1) + '. ' + (it.name || it.id) + (mt ? ' ('+mt+')' : '');
      }).join('\n');
      const ans = prompt('Importar da biblioteca — digite o número do .tile:\n\n' + lines, '1');
      if(ans === null) return;
      ...
      const loaded = await AssetLibrary.load('tile', items[idx].id);
      const doc = loaded.data || loaded;
      nmsImportApplyJson(doc, loaded.name || items[idx].name || items[idx].id, true);
      if(typeof Project !== 'undefined' && Project.status){
        Project.status('Biblioteca: carregado "' + (loaded.name || items[idx].name || items[idx].id) + '"');
      }
    }catch(err){
      console.error(err);
      alert(err.message || 'Falha ao carregar da biblioteca');
    }
  }
    ...
    if(isChr){
      ...
      reader.onload = (ev)=>{
        try{
          ...
          nmsImportApplyJson(json, file.name, isTile);
        }catch(err){
  ...
  return {
    ...
    loadBuffer(buf, pals, bankData){
      ...
      let newBuf;
      if(buf && buf.length){
        const pages = Math.max(2, Math.ceil(buf.length/4096));
        newBuf = new Uint8Array(pages*4096);
        newBuf.set(buf.length > pages*4096 ? buf.subarray(0, pages*4096) : buf);
      } else {
        newBuf = new Uint8Array(8192);
      }
      ...
      // Metadados de páginas (NMS.chrPages) — compat: se ausente, deriva do tamanho
      if(bankData && Array.isArray(bankData.chrPages) && bankData.chrPages.length){
        chrPages = bankData.chrPages.map((e,i)=>{
    ...
    addBank(){ return this.addPage(); },
    addPage(){
      ensureChrPages();
      const nb = new Uint8Array(chrBuffer.length + 4096);
      nb.set(chrBuffer);
      chrBuffer = nb;
      const i = chrPages.length;
      chrPages.push({ name: 'Página '+i, role: (i%2===0?'sprite':'background') });
      currentBank = i;
      updateBankSelect();
      ensurePaletteMatchesBank();
      renderAll();
    ...
    removePage(){
      ensureChrPages();
      if(currentBank < 2){
        alert('As páginas 0 (Sprites) e 1 (Backgrounds) não podem ser removidas.');
        return;
      }
      const total = chrPages.length;
      if(total <= 2) return;
      if(!confirm('Remover a página '+currentBank+' ("'+(chrPages[currentBank]?.name||'')+'")?\nTiles e referências a esta página podem quebrar.')) return;
      const page = currentBank;
      const nb = new Uint8Array((total-1)*4096);
      // copia páginas antes e depois
      if(page > 0) nb.set(chrBuffer.subarray(0, page*4096), 0);
      if(page < total-1) nb.set(chrBuffer.subarray((page+1)*4096), page*4096);
      chrBuffer = nb;
      chrPages.splice(page, 1);
      // Ajusta metatiles que apontavam para páginas removidas/deslocadas
      metatiles.forEach(mt=>{
        const b = mt.bank|0;
        if(b === page) mt.bank = 0;
        else if(b > page) mt.bank = b - 1;
      });
      currentBank = Math.min(page, chrPages.length-1);
      updateBankSelect();
      ensurePaletteMatchesBank();
      updateMetatileSelect();
      renderAll();
    ...
    openNmsImport, closeNmsImport, nmsImportFromLibrary, nmsImportSetBank, nmsImportClearPicked, nmsImportConfirm, beginColorSwap
  };
})();
