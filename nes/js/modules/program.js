// PROGRAM MODULE v0.2.0 - Esboço inicial: Variáveis, Eventos e Regras (a "linguagem" do jogo)
...
const PROGRAM = (() => {
  ...
  let selectedRuleTabId = 'main'; // abas de UI dentro de Regras (não afeta runtime)
  ...
  const ACTION_CATALOG = {
    ...
    shoot:       { label: 'Atirar' },
    pause_game:  { label: 'Pausar o jogo' },
  ...
  // objetos nomeados (Dano/Warp/Spawn, cadastrados na aba Objetos) e hitboxes de personagens
  ...
  function allHitboxRefs(){
    ...
    (Project.data?.characters||[]).forEach(c=>{
      (c.hitboxes||[]).forEach(hb=>{
        const icon = hb.type==='attack' ? '⚔️' : (hb.type==='hurt' ? '💢' : '🟦');
        refs.push({ value:`char:${c.id}:${hb.id}`, label:`${icon} ${c.name}: ${hb.name}` });
      });
    });
    return refs;
  }
  ...
  // ---------------- REGRAS ----------------
  /** Abas só de organização visual. Não mudam comportamento das regras no build. */
  function ensureRuleTabs(){
    if(typeof Project === 'undefined' || !Project.data) return [{ id:'main', name:'main' }];
    if(!Array.isArray(Project.data.ruleTabs) || !Project.data.ruleTabs.length){
      Project.data.ruleTabs = [{ id:'main', name:'main' }];
    }
    // garante "main"
    if(!Project.data.ruleTabs.some(t => t.id === 'main')){
      Project.data.ruleTabs.unshift({ id:'main', name:'main' });
    }
    (Project.data.rules || []).forEach(r => {
      if(!r.uiTabId) r.uiTabId = 'main';
    });
    return Project.data.ruleTabs;
  }
  function renderRuleTabsBar(){
    const tabs = ensureRuleTabs();
    if(!tabs.some(t => t.id === selectedRuleTabId)) selectedRuleTabId = 'main';
    const btns = tabs.map(tab => {
      const on = tab.id === selectedRuleTabId;
      return `<button type="button"
        class="btn-tool"
        onclick="PROGRAM.selectRuleTab('${tab.id}')"
        oncontextmenu="event.preventDefault();PROGRAM.renameRuleTab('${tab.id}')"
        title="Botão direito: renomear aba"
        style="font-size:11px;padding:4px 12px;border-radius:4px 4px 0 0;border:1px solid ${on?'#ffcc00':'#444'};border-bottom:${on?'1px solid #1e1e1e':'1px solid #333'};background:${on?'#3a3a10':'#252526'};color:${on?'#ffcc00':'#aaa'};margin-bottom:-1px">${escHtml(tab.name)}</button>`;
    }).join('');
    return `
      <div style="display:flex;align-items:flex-end;gap:2px;border-bottom:1px solid #333;margin-bottom:12px;flex-wrap:wrap">
        ${btns}
        <button type="button" class="btn-tool" onclick="PROGRAM.addRuleTab()" title="Nova aba de regras"
          style="font-size:14px;padding:2px 10px;border-radius:4px 4px 0 0;border:1px dashed #555;background:#1a1a1a;color:#888;margin-bottom:-1px">+</button>
      </div>`;
  }
  function escHtml(s){
  ...
  function renderRulesTab(){
    const tabs = ensureRuleTabs();
    if(!tabs.some(t => t.id === selectedRuleTabId)) selectedRuleTabId = 'main';
    const allRules = Project.data?.rules || [];
    // Filtra só para UI — array completo em Project.data.rules continua intacto
    const rules = allRules.filter(r => (r.uiTabId || 'main') === selectedRuleTabId);
    const phases = Project.data?.phases || [];
    const listHtml = rules.map(r => `
      <div onclick="PROGRAM.selectRule('${r.id}')"
           oncontextmenu="event.preventDefault();PROGRAM.moveRuleToTab('${r.id}')"
           title="Botão direito: mover para outra aba"
           style="padding:8px;border-radius:4px;cursor:pointer;background:${r.id===selectedRuleId?'#3a3a10':'#181818'};border:1px solid ${r.id===selectedRuleId?'#ffcc00':'#333'};margin-bottom:4px">
        <div style="color:#fff;font-size:11px">${escHtml(r.name)}</div>
        <div style="color:#888;font-size:9px">${r.scope==='global'?'Global':'Fase: '+(phases.find(p=>p.id===r.scope)?.name||'?')} • ${(r.steps||[]).length} passo(s)</div>
      </div>`).join('');
    const selectedRule = allRules.find(r => r.id === selectedRuleId && (r.uiTabId||'main') === selectedRuleTabId)
      || null;
    return `
      <div style="display:flex;flex-direction:column;height:100%;min-height:0">
        ...
        <div style="display:flex;gap:14px;flex:1;min-height:0">
          <div style="width:260px;display:flex;flex-direction:column;gap:8px">
            ...
            <div style="overflow:auto;flex:1">${listHtml || '<div style="color:#666;font-size:11px">Nenhuma regra nesta aba.</div>'}</div>
          </div>
          <div style="flex:1;background:#111;border:1px solid #333;border-radius:6px;padding:12px;overflow:auto">
            ${selectedRule ? renderRuleEditor(selectedRule, phases) : '<div style="color:#666;font-size:11px">Selecione ou crie uma regra nesta aba.</div>'}
          </div>
        </div>
      </div>`;
  }
  ...
  function renderStepRow(rule, step, idx, vars, events){
    ...
    if(step.type === 'if_event'){
      const selEv = events.find(e => e.id === step.eventId);
      const isIdle = selEv && (selEv.button === 'P1-IDLE' || selEv.button === 'P2-IDLE' ||
        /P[12]-IDLE/i.test(selEv.name||'') || /P[12]-IDLE/i.test(selEv.button||''));
      ...
      if(isIdle){
        fields += `<label style="font-size:9px;color:#888;display:flex;align-items:center;gap:4px">tempo (frames)
          <input type="number" min="0" max="65535" value="${step.idleTime ?? 0}" title="Frames parado antes de disparar (0 = imediato)"
            onchange="PROGRAM.updateStep('${rule.id}',${idx},'idleTime',Math.max(0,parseInt(this.value)||0))"
            style="width:64px;background:#000;color:#4ec9b0;border:1px solid #444;border-radius:4px;padding:4px;font-size:10px;font-family:monospace"></label>`;
      }
    } else if(step.type === 'if_hitbox'){
      const refs = allHitboxRefs();
      const selStyleH = "background:#000;color:#fff;border:1px solid #444;border-radius:4px;padding:4px;font-size:10px";
      const opts = sel => `<option value="">— hitbox —</option>` + refs.map(r=>`<option value="${r.value}" ${sel===r.value?'selected':''}>${r.label}</option>`).join('');
      ...
      if(step.actionId === 'goto_warp'){
      ...
      } else if(step.actionId === 'set_on_ground'){
        const chars = Project.data?.characters || [];
        fields += `<select onchange="PROGRAM.updateStep('${rule.id}',${idx},'targetId',this.value)" style="${selStyle}">
          <option value="">— personagem —</option>${chars.map(c=>`<option value="${c.id}" ${step.targetId===c.id?'selected':''}>${c.name}</option>`).join('')}</select>
          <select onchange="PROGRAM.updateStep('${rule.id}',${idx},'value',this.value)" style="${selStyle}">
            <option value="1" ${step.value=='1'||step.value===undefined?'selected':''}>Ativar (no chão)</option>
        ...
        const levels = Project.data?.speedLevels || []; const chars = Project.data?.characters || [];
        fields += `<select onchange="PROGRAM.updateStep('${rule.id}',${idx},'targetId',this.value)" style="${selStyle}">
        ...
        if(step.direction === 'zigzag'){
          fields += `<label style="font-size:9px;color:#888;display:flex;align-items:center;gap:3px">range
            <input type="number" min="1" max="255" value="${step.zigzagRange ?? 32}" title="Alcance do zig-zag em pixels"
              onchange="PROGRAM.updateStep('${rule.id}',${idx},'zigzagRange',Math.max(1,Math.min(255,parseInt(this.value)||32)))"
              style="width:52px;background:#000;color:#4ec9b0;border:1px solid #444;border-radius:4px;padding:4px;font-size:10px;font-family:monospace"></label>`;
        }
        fields += `<select onchange="PROGRAM.updateStep('${rule.id}',${idx},'animId',this.value)" style="${selStyle}">
        ...
        const dirs = [
          ...
          { value:'left', label:'Esquerda' },
          { value:'right', label:'Direita' },
          { value:'front', label:'Frente (olhar do personagem)' }
        ];
        const flips = [
          { value:'default', label:'Default' },
          { value:'flip_v', label:'Flip V' },
        ...
        fields += `<select onchange="PROGRAM.updateStep('${rule.id}',${idx},'charId',this.value)" style="${selStyle}">
          ...
          <select onchange="PROGRAM.updateStep('${rule.id}',${idx},'flip',this.value)" style="${selStyle}">
            ${flips.map(f=>`<option value="${f.value}" ${(step.flip||'default')===f.value?'selected':''}>${f.label}</option>`).join('')}</select>`;
      } else if(step.actionId === 'pause_game'){
        fields += `<span style="font-size:9px;color:#666">Alterna pausa do jogo (ex.: botão Start)</span>`;
  ...
  function addRule(){
    ...
    ensureRuleTabs();
    const r = { id:'rule_'+Date.now(), name: name.trim(), scope:'global', steps:[], uiTabId: selectedRuleTabId || 'main' };
    Project.data.rules.push(r); selectedRuleId = r.id; renderTab();
  }
  function selectRuleTab(id){
    selectedRuleTabId = id || 'main';
    selectedRuleId = null;
    renderTab();
  }
  function addRuleTab(){
    ensureRuleTabs();
    const name = prompt('Nome da nova aba de regras:', `aba_${Project.data.ruleTabs.length+1}`);
    if(!name || !name.trim()) return;
    const id = 'rtab_'+Date.now();
    Project.data.ruleTabs.push({ id, name: name.trim() });
    selectedRuleTabId = id;
    selectedRuleId = null;
    renderTab();
  }
  function renameRuleTab(id){
    ensureRuleTabs();
    const tab = Project.data.ruleTabs.find(t => t.id === id);
    if(!tab) return;
    const name = prompt(`Renomear aba\n\nNome atual: ${tab.name}\n\nNovo nome:`, tab.name);
    if(name === null || !String(name).trim()) return;
    tab.name = String(name).trim();
    renderTab();
  }
  /** Botão direito na regra: realoca para outra aba (só UI). */
  function moveRuleToTab(ruleId){
    ensureRuleTabs();
    const r = (Project.data.rules || []).find(x => x.id === ruleId);
    if(!r) return;
    const tabs = Project.data.ruleTabs;
    const list = tabs.map((tab, i) => `${i + 1} - ${tab.name}`).join('\n');
    const curIdx = Math.max(0, tabs.findIndex(tab => tab.id === (r.uiTabId || 'main')));
    const ans = prompt(
      `Mover a regra "${r.name}" para qual aba?\n\n${list}\n\nDigite o número da aba:`,
      String(curIdx + 1)
    );
    if(ans === null) return;
    const n = parseInt(ans, 10);
    if(isNaN(n) || n < 1 || n > tabs.length){
      alert(`Número inválido. Use de 1 a ${tabs.length}.`);
      return;
    }
    r.uiTabId = tabs[n - 1].id;
    // se tirou da aba atual, some da lista
    if(r.uiTabId !== selectedRuleTabId && selectedRuleId === r.id) selectedRuleId = null;
    renderTab();
    if(typeof Project !== 'undefined' && Project.status){
      Project.status(`Regra "${r.name}" → aba "${tabs[n - 1].name}"`);
    }
  }
  ...
  function updateStep(ruleId, idx, field, value){
    ...
    const act = r.steps[idx].actionId;
    if(field === 'type' || field === 'actionId' || field === 'eventId' || field === 'direction' ||
       (field === 'charId' && (act === 'move_character' || act === 'shoot'))){
  ...
  return {
    ...
    selectRuleTab, addRuleTab, renameRuleTab, moveRuleToTab
  };
})();
