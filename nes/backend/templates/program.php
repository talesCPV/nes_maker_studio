...
return [
    'program_vars_zp' => static function (array $ctx): string {
        ...
        $lines[] = 'pv_char_scrx:   .res 1  ; Fase 9 fix: posicao-na-tela (ja subtraida) da instancia A/heroi-alvo, antes de somar o offset da hitbox';
        $lines[] = 'pv_char_scrx2:  .res 1  ; Fase 9 fix: mesma coisa pro lado B do check_char_char_hit';
    ...
    'program_hitbox_engine' => static function (array $ctx): string {
        ...
        $bBottom = $by + $bh; // Fase 9 fix: antes hardcoded #16 - ver check_ground em system.php
        $bCenterX = $bx + intdiv($bw, 2);
        $lines = [];
        ...
        $lines[] = '; devolve A=1; senao A=0. Fase 9 fix: mesma logica de visibilidade de';
        $lines[] = '; update_instances_oam (posicao continua tela*256+x vs play_idx*256+';
        $lines[] = '; scroll_x) - antes usava so inst_x-scroll_x, que pra uma instancia da';
        $lines[] = '; PROXIMA tela (adiantada por spawn_append_screen, ainda invisivel)';
        $lines[] = '; podia dar um resultado sem "borrow" mas errado, calculando uma posicao';
        $lines[] = '; bogus que parecia sobrepor o heroi (o hitbox "passava por cima" dele).';
        ...
        $lines[] = '  LDA inst_x,X';
        $lines[] = '  SBC scroll_x';
        $lines[] = '  STA pv_char_scrx';
        $lines[] = '  LDA inst_screen,X';
        $lines[] = '  SBC play_idx';
        $lines[] = '  BEQ cchh_onscreen';
        $lines[] = '  BMI cchh_next';
        $lines[] = '  JMP cchh_next';
        $lines[] = 'cchh_onscreen:';
        $lines[] = '  LDA pv_char_scrx';
        $lines[] = '  CLC';
        ...
        $lines[] = '  LDA inst_screen,X';
        $lines[] = '  SBC play_idx';
        $lines[] = '  BEQ cccc_a_onscreen';
        $lines[] = '  BMI cccc_a_next';
        $lines[] = '  JMP cccc_a_next';
        $lines[] = 'cccc_a_onscreen:';
        $lines[] = '  LDA pv_char_scrx';
        $lines[] = '  CLC';
        $lines[] = '  ADC pv_char_hb_x';
        $lines[] = '  STA pv_hbA_x';
        ...
        $lines[] = '  LDA inst_x,Y';
        $lines[] = '  SBC scroll_x';
        $lines[] = '  STA pv_char_scrx2';
        $lines[] = '  LDA inst_screen,Y';
        $lines[] = '  SBC play_idx';
        $lines[] = '  BEQ cccc_b_onscreen';
        $lines[] = '  BMI cccc_b_next';
        $lines[] = '  JMP cccc_b_next';
        $lines[] = 'cccc_b_onscreen:';
        $lines[] = '  LDA pv_char_scrx2';
        $lines[] = '  CLC';
