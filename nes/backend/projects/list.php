...
try {
    ...
    foreach ($rows as $row) {
        ...
        if ($sys !== null && $sys !== '') {
            $isMatch = (strcasecmp((string) $sys, 'NES') === 0);
        } else {
            $isMatch = (bool) preg_match('/\.nms$/i', $filename);
        }
    ...
    error_log('NGC Project List Error: ' . $e->getMessage());
