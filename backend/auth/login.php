...
$data=json_decode(file_get_contents('php://input')?:'',true);if(!is_array($data))response(['success'=>false,'message'=>'JSON inválido.'],400);
...
try{
 $pdo=db();$q=$pdo->prepare('SELECT id,name,email,password_hash,status FROM users WHERE email=:email LIMIT 1');$q->execute([':email'=>$email]);$user=$q->fetch();
 ...
 session_start();session_regenerate_id(true);$_SESSION['user_id']=(int)$user['id'];$_SESSION['user_name']=$user['name'];$_SESSION['user_email']=$user['email'];$_SESSION['authenticated']=true;
 $q=$pdo->prepare('UPDATE users SET last_login_at=NOW() WHERE id=:id');$q->execute([':id'=>$user['id']]);
 response(['success'=>true,'message'=>'Login realizado com sucesso.','user'=>['id'=>(int)$user['id'],'name'=>$user['name'],'email'=>$user['email']]]);
}catch(Throwable $e){error_log('NGC Login Error: '.$e->getMessage());response(['success'=>false,'message'=>'Não foi possível realizar o login.'],500);}
