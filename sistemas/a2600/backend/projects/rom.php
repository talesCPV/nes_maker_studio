...
/**
 * GET backend/a2600/projects/rom.php?id=N
 * Serve data/users/{uid}/a2600/projects/{id}/game.bin (somente dono).
 * Não é repositório público — exige sessão.
 */

require_once dirname(__DIR__, 4) . '/backend/auth/auth_check.php';
require_once dirname(__DIR__, 4) . '/backend/config/database.php';
...
try {
    ...
    $path = dirname(__DIR__, 4) .
        ...
        DIRECTORY_SEPARATOR . 'a2600' . DIRECTORY_SEPARATOR . 'projects' .
        DIRECTORY_SEPARATOR . $projectId .
        DIRECTORY_SEPARATOR . 'game.bin';
    ...
    if (!is_file($path)) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Este projeto ainda não tem game.bin. Faça um Build ROM no editor.',
        ]);
        exit;
    }
