...
const SPRITES = (() => {
  ...
  let frameIndex = 0; // frame de animação ativo no editor
  let pixels = null;
  ...
  /** Garante sprite.frames[] (animação). Mantém data/lineColors como espelho do frame ativo. */
  function ensureFrames(s) {
    if (!s) return [];
    if (!Array.isArray(s.frames) || !s.frames.length) {
      s.frames = [
        {
          data: s.data || toB64(emptyPixels(Math.max(1, s.height | 0) || H_DEFAULT)),
          lineColors:
            s.lineColors ||
            lineColorsToB64(new Uint8Array(Math.max(1, s.height | 0) || H_DEFAULT).fill((s.color != null ? s.color : 0x2a) & 0xfe)),
          duration: Math.max(1, s.animSpeed | 0 || 4),
        },
      ];
    }
    s.frames.forEach((f) => {
      if (f.duration == null || f.duration < 1) f.duration = 4;
    });
    if (s.animSpeed == null) s.animSpeed = 4;
    return s.frames;
  }

  function currentFrame(s) {
    const frames = ensureFrames(s);
  ...
  function ensureSelection() {
    const arr = list();
    if (!arr.length) {
      const empty = toB64(emptyPixels(H_DEFAULT));
      const lc = lineColorsToB64(new Uint8Array(H_DEFAULT).fill(0x2a));
      const s = {
        id: uid(),
        name: 'Player 0',
        player: 0,
        ...
        data: empty,
        lineColors: lc,
        frames: [{ data: empty, lineColors: lc, duration: 4 }],
      };
      arr.push(s);
      selectedId = s.id;
    }
  ...
  function loadSelected() {
    ...
    nusiz = s.nusiz | 0;
    player = s.player | 0;
    const fr = currentFrame(s);
    pixels = fromB64(fr.data || s.data, height);
    lineColors = lineColorsFromB64(fr.lineColors || s.lineColors, height, color);
  }
  ...
  function persist() {
    ...
    s.nusiz = nusiz | 0;
    s.player = player | 0;
    const payload = toB64(pixels);
    const lc = lineColorsToB64(lineColors || new Uint8Array(height));
    s.data = payload;
    s.lineColors = lc;
    const frames = ensureFrames(s);
  ...
  function stopAnimPreview() {
    ...
    const btn = document.getElementById('spAnimPlay');
  ...
  function selectFrame(i) {
    ...
    persist();
    const frames = ensureFrames(s);
    frameIndex = Math.max(0, Math.min(frames.length - 1, i | 0));
    loadSelected();
    undoStack.length = 0;
    // atualiza UI de frames sem rebuild completo se possível
    const strip = document.getElementById('spFrameStrip');
    if (strip) {
      strip.querySelectorAll('.sp-frame').forEach((el, idx) => {
        el.classList.toggle('active', idx === frameIndex);
      });
      const lab = document.getElementById('spFrameLabel');
      if (lab) lab.textContent = 'Frame ' + (frameIndex + 1) + ' / ' + frames.length;
      const dur = document.getElementById('spFrameDur');
      if (dur) dur.value = frames[frameIndex].duration || 4;
    }
    resizeCanvas();
    redraw();
    drawThumbs();
  ...
  function addFrame(dup) {
    ...
    const neo = {
      data: dup ? src.data : toB64(emptyPixels(height)),
      lineColors: dup
        ? src.lineColors
        : lineColorsToB64(new Uint8Array(height).fill(color & 0xfe)),
      duration: src.duration || 4,
    };
    frames.splice(frameIndex + 1, 0, neo);
    frameIndex = frameIndex + 1;
    loadSelected();
    undoStack.length = 0;
    buildHTML();
    if (typeof Project.status === 'function') Project.status('frame adicionado — salve o projeto');
  }
  ...
  function deleteFrame() {
    ...
    const frames = ensureFrames(s);
    if (frames.length <= 1) {
      alert('Precisa de pelo menos 1 frame.');
      return;
    }
    if (!confirm('Remover este frame da animação?')) return;
    frames.splice(frameIndex, 1);
    if (frameIndex >= frames.length) frameIndex = frames.length - 1;
    // espelho root
    const fr = frames[frameIndex];
    s.data = fr.data;
    s.lineColors = fr.lineColors;
    loadSelected();
    undoStack.length = 0;
    buildHTML();
    if (typeof Project.status === 'function') Project.status('frame removido — salve o projeto');
  }
  ...
  function drawFrameThumbs() {
    ...
    if (!s) return;
    const frames = ensureFrames(s);
    document.querySelectorAll('canvas[data-frame-thumb]').forEach((cv) => {
      const idx = parseInt(cv.getAttribute('data-frame-thumb'), 10);
      const fr = frames[idx];
      if (!fr) return;
      const h = Math.max(1, s.height | 0);
      const pix = fromB64(fr.data, h);
      const lcols = lineColorsFromB64(fr.lineColors, h, s.color || 0x2a);
    ...
    const listHtml = arr
      ...
      <div class="sp-item ${sp.id === selectedId ? 'active' : ''}" data-id="${sp.id}">
        ...
        <div class="sp-item-meta">
          ...
          <div class="sp-item-sub">P${sp.player | 0} · ${sp.height | 0} linhas · ${(sp.frames && sp.frames.length) || 1} frame(s)</div>
        </div>
      </div>`
      )
      .join('');
  ...
  function renderFrameStrip() {
    ...
    strip.innerHTML = frames
      .map(
        (f, i) =>
          `<button type="button" class="sp-frame ${i === frameIndex ? 'active' : ''}" data-frame="${i}" title="Frame ${i + 1}">
            <canvas width="28" height="28" data-frame-thumb="${i}"></canvas>
            <span>${i + 1}</span>
          </button>`
      )
      .join('');
    if (lab) lab.textContent = 'Frame ' + (frameIndex + 1) + ' / ' + frames.length;
    if (dur) dur.value = frames[frameIndex].duration || 4;
    strip.querySelectorAll('.sp-frame').forEach((btn) => {
      btn.addEventListener('click', () => selectFrame(parseInt(btn.getAttribute('data-frame'), 10)));
    });
  }
  ...
  function bind() {
    document.getElementById('spAdd')?.addEventListener('click', () => {
      persist();
      const n = list().length;
      const col = n % 2 ? 0x4a : 0x2a;
      const empty = toB64(emptyPixels(H_DEFAULT));
      const lc = lineColorsToB64(new Uint8Array(H_DEFAULT).fill(col));
      const s = {
        id: uid(),
        name: 'Sprite ' + (n + 1),
        player: n % 2,
    ...
    document.getElementById('spFrameAdd')?.addEventListener('click', () => addFrame(false));
    document.getElementById('spFrameDup')?.addEventListener('click', () => addFrame(true));
    document.getElementById('spFrameDel')?.addEventListener('click', () => deleteFrame());
    document.getElementById('spFrameDur')?.addEventListener('change', (e) => {
      const s = find(selectedId);
      if (!s) return;
      const frames = ensureFrames(s);
      const fr = frames[frameIndex];
      ...
      e.target.value = v;
      fr.duration = v;
      if (typeof Project.status === 'function') Project.status('duração do frame — salve o projeto');
    });
    document.getElementById('spAnimPlay')?.addEventListener('click', () => {
      const s = find(selectedId);
      if (!s) return;
      const frames = ensureFrames(s);
      if (frames.length < 2) {
        if (typeof Project.status === 'function') Project.status('adicione mais frames para animar');
        return;
      }
      if (animPreviewTimer) {
        stopAnimPreview();
      ...
      if (btn) btn.textContent = '■';
      let ticks = 0;
      let localIdx = frameIndex;
      animPreviewTimer = setInterval(() => {
        const fr = frames[localIdx];
        const dur = Math.max(1, (fr && fr.duration) || 4);
        ticks++;
        if (ticks >= dur) {
          ticks = 0;
          localIdx = (localIdx + 1) % frames.length;
          // preview sem persistir edição atual a cada tick de forma destrutiva
          frameIndex = localIdx;
          const f = frames[localIdx];
          pixels = fromB64(f.data, height);
          lineColors = lineColorsFromB64(f.lineColors, height, color);
          const strip = document.getElementById('spFrameStrip');
          strip?.querySelectorAll('.sp-frame').forEach((el, idx) => el.classList.toggle('active', idx === localIdx));
          const lab = document.getElementById('spFrameLabel');
          if (lab) lab.textContent = 'Frame ' + (localIdx + 1) + ' / ' + frames.length;
          redraw();
        }
      }, 50); // ~20 fps de tick de editor; duration conta ticks
    });
    ...
    document.querySelectorAll('.sp-item[data-id]').forEach((el) => {
      el.addEventListener('click', () => {
        persist();
        stopAnimPreview();
