...
(function(){
  ...
  function pushUndo(){
    try{
      undoStack.push({
        chrData: new Uint8Array(chrData),
        metatilesList: JSON.parse(JSON.stringify(metatilesList)),
        metatileSlots: [...metatileSlots],
        metatileCols,
        metatileRows
      });
  ...
  function doUndo(){
    ...
    try{
      ...
      chrData.set(prev.chrData);
      metatilesList=prev.metatilesList;
      metatileSlots=prev.metatileSlots;
      metatileCols=prev.metatileCols;
      metatileRows=prev.metatileRows;
      syncMetatilesToProject();
  ...
  function doRedo(){
    ...
    try{
      ...
      chrData.set(next.chrData);
      metatilesList=next.metatilesList;
      metatileSlots=next.metatileSlots;
      metatileCols=next.metatileCols;
      metatileRows=next.metatileRows;
      syncMetatilesToProject();
      if(window.AppState) window.AppState.markDirty();
      render();
    }catch(e){}
  ...
  function render(){
    ...
    containerEl.querySelectorAll('.chr-tool-btn[data-action], .chr-tbtn[data-action]').forEach(btn=>{
      btn.onclick=()=>{
        ...
        if(['left','right','up','down','flipH','flipV','rotate','clear','fillAll','rect','circle','clearAll'].includes(act)){ try{ pushUndo(); }catch{} }
