...
(function(){
  ...
  function render(){
      ...
      <div class="bg-editor-nes-clone">
        ...
        <div class="chr-tools-top">
          <div class="chr-tools-left">
            ...
            <button class="chr-tool-btn ${tool==='pen'?'active':''}" data-tool="pen" title="Pen" style="width:28px;height:24px;">✏️</button>
            <button class="chr-tool-btn ${tool==='eraser'?'active':''}" data-tool="eraser" title="Eraser" style="width:28px;height:24px;">🧹</button>
            <button class="chr-tool-btn ${tool==='picker'?'active':''}" data-tool="picker" title="Picker" style="width:28px;height:24px;">💉</button>
            <button class="chr-tool-btn ${tool==='fill'?'active':''}" data-tool="fill" title="Fill" style="width:28px;height:24px;">🪣</button>
            <span class="chr-sep"></span>
            ...
            <label class="chr-check">Paleta: <select id="bg-palette" class="chr-select small"><option value="0" ${selectedPalette===0?'selected':''}>Pal 0</option><option value="1" ${selectedPalette===1?'selected':''}>Pal 1</option><option value="2" ${selectedPalette===2?'selected':''}>Pal 2</option><option value="3" ${selectedPalette===3?'selected':''}>Pal 3</option></select></label>
            <span class="chr-sep"></span>
            <button class="chr-tbtn" id="bg-undo-btn" style="background:#3a2a1a;border-color:#5a4a2a;color:#e8c36a;">↩️ Undo</button>
            <button class="chr-tbtn" id="bg-redo-btn">↪️ Redo</button>
            <button class="chr-btn small" data-action="clearMap">🗑️ Limpar Mapa</button>
            <button class="chr-btn small" data-action="fillMap">🪣 Preencher</button>
            <span class="chr-sep"></span>
        ...
        <div class="bg-main">
          ...
          <!-- PAINEL DIREITO SIMPLIFICADO - V20 ferramentas movidas para topbar -->
          <div class="bg-tools-panel">
            ...
            <div class="bg-tools-section">
              <div class="bg-tools-title">INFO</div>
              <div style="font-size:10px;color:#888;line-height:1.5;">
                <div>Plane: ${currentPlane}</div>
                <div>Metatile: ${selectedMetatileId||'—'}</div>
                <div>Tool: ${tool}</div>
                <div>Pal: ${selectedPalette}</div>
                <div>Cells: ${mapWidth*mapHeight}</div>
                <div>Undo: ${bgUndoStack.length}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    const bgUndoBtn = containerEl.querySelector('#bg-undo-btn');
    const bgRedoBtn = containerEl.querySelector('#bg-redo-btn');
    if(bgUndoBtn) bgUndoBtn.onclick=()=>bgDoUndo();
    if(bgRedoBtn) bgRedoBtn.onclick=()=>bgDoRedo();
    ...
    containerEl.querySelectorAll('[data-action]').forEach(btn=>{
      ...
      if(act==='clearMap'){ btn.onclick=()=>{ if(confirm('Limpar mapa?')){ try{ bgPushUndo(); }catch{} mapData=Array.from({length:mapHeight},()=>Array(mapWidth).fill(null)); if(window.AppState){ window.AppState.project.backgrounds={width:mapWidth,height:mapHeight,map:mapData}; window.AppState.markDirty(); } render(); } }; }
      if(act==='fillMap'){ btn.onclick=()=>{ if(!selectedMetatileId){ alert('Selecione um metatile'); return; } try{ bgPushUndo(); }catch{} for(let y=0;y<mapHeight;y++) for(let x=0;x<mapWidth;x++) mapData[y][x]={metatileId:selectedMetatileId, palette:selectedPalette}; if(window.AppState){ window.AppState.project.backgrounds={width:mapWidth,height:mapHeight,map:mapData}; window.AppState.markDirty(); } render(); }; }
    });
