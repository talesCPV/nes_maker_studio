...
try {
    ...
    // SQL em aspas duplas para permitir 'NES' / '%.nms' dentro da query
    $stmt = $pdo->prepare(
        "SELECT
            id,
            parent_project_id,
         ...
         WHERE user_id = :user_id
           AND is_deleted = 1
           AND (`system` = 'NES' OR (`system` IS NULL AND filename LIKE '%.nms'))
    ...
    error_log('NGC Project Trash Error: ' . $e->getMessage());
    response([
        'success' => false,
