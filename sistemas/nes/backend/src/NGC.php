...
final class NGC
{
    ...
    public function build(array $request): array
    {
        ...
        // Camada 9 (limpeza pré-áudio): sprite_chr/background_chr (templates que só
        // renderizavam o banco 0, mantidos só pro debug=true) foram removidos - quem
        // realmente vai pro .asm final é charsSegments, que emite 1 segmento (NROM) ou
        ...
        $result = [
            ...
            'asm' => $builder->build(),
            'migrated' => ['header', 'vectors', 'zeropage', 'nmi', 'reset', 'input', 'main_loop', 'collision', 'player', 'player_manual_move', 'scroll', 'background', 'background_tables', 'background_data', 'sprite_player', 'sprite_entities', 'sprite_data', 'chars_segments', 'music', 'game_flow', 'palette_data', 'program_vars', 'program_rules', 'program_hitbox_engine'],
        ];
        ...
        if (!empty($request['debug'])) {
            $result['blocks'] = [
                ...
                'game_flow' => $gameFlow, 'palette_data' => $paletteData, 'music' => $music,
            ];
        }
