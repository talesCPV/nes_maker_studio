...
const Project = {
  ...
  async newProject(silent=false){
    ...
    if(typeof UI !== 'undefined' && UI.switchModule) UI.switchModule('config');
  },
  loadIntoEditors(){
    ...
    if(this.data.phases && typeof CONFIG !== 'undefined'){
      try{ CONFIG.loadPhases(this.data.phases); }catch(e){}
    }
    // Fases já em Project.data — repopula select do módulo Backgrounds
    if(typeof BG !== 'undefined' && BG.refreshPhaseAndMetatiles){
      try{ BG.refreshPhaseAndMetatiles(); }catch(e){}
    }
    // Sound v2
  ...
  collectProjectData(){
    ...
    if(typeof CHR !== 'undefined'){
      ...
      if(CHR.getChrPages) this.data.chrPages = CHR.getChrPages();
    }
    ...
    // Level Design: grava o grid atual (incl. telas posicionadas) nas fases ANTES do dump
    try{
      if(typeof LEVEL_DESIGN !== 'undefined' && LEVEL_DESIGN.flushToProject){
        LEVEL_DESIGN.flushToProject();
      } else if(typeof LEVEL_DESIGN !== 'undefined' && LEVEL_DESIGN.persistLevelMap){
        LEVEL_DESIGN.persistLevelMap();
      }
    }catch(e){ console.warn('LEVEL_DESIGN flush:', e); }
    if(typeof CONFIG !== 'undefined' && CONFIG.getPhases){
      try{ this.data.phases = CONFIG.getPhases(); }catch(e){}
    }
  ...
  async createProjectOnBackend(name, description, parentProjectId){
    ...
    const response = await fetch(APP('/sistemas/nes/backend/projects/create.php'), {
      method: 'POST',
  ...
  async uploadThumbnailIfMissing(dataUrl){
    ...
    try {
      const response = await fetch(APP('/sistemas/nes/backend/projects/thumbnail.php'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({
  ...
  async saveProjectToBackend(projectId, nms){
    const response = await fetch(APP('/sistemas/nes/backend/projects/save.php'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'same-origin',
      body: JSON.stringify({
  ...
  async loadFromFile(file, opts = {}){
    const skipModuleSwitch = !!opts.skipModuleSwitch;
    ...
    try{
      ...
      // Páginas CHR (4KB): compat com .nms antigos (só chr[])
      if(!Array.isArray(json.chrPages) || !json.chrPages.length){
        const n = Math.max(2, Math.ceil((json.chr?.length||8192)/4096));
        json.chrPages = [];
        for(let i=0;i<n;i++){
          if(i===0) json.chrPages.push({ name:'Sprites', role:'sprite' });
          else if(i===1) json.chrPages.push({ name:'Backgrounds', role:'background' });
          else json.chrPages.push({ name:'Página '+i, role:(i%2===0?'sprite':'background') });
        }
      } else {
        json.chrPages = json.chrPages.map((e,i)=>{
          if(i===0) return { name:(e?.name||'Sprites'), role:'sprite' };
          if(i===1) return { name:(e?.name||'Backgrounds'), role:'background' };
          const role = (e?.role==='background'||e?.role==='sprite') ? e.role : (i%2===0?'sprite':'background');
          return { name:(e?.name||('Página '+i)), role };
        });
      }
      ...
      setTimeout(()=>{
        ...
        // Carregado a partir do backend (dashboard -> editor.html?project=ID):
        // quem chamou (loadProjectFromBackend, via editor.html) já troca pro
        // módulo certo de forma determinística depois que TUDO termina de
        // carregar. Rodar de novo aqui, num timer solto, só criava uma
        // corrida entre 2 chamadas de switchModule - a 2a (essa) podia
        // vencer com o nome errado (bug do rename dashboard->config: essa
        // sobrava com 'dashboard' enquanto editor.html já usava 'config',
        // limpando o .active do módulo certo sem recolocar em lugar nenhum -
        // tela em branco até clicar no menu manualmente). Corrigido restrigindo
        // esse switch pro caso de import local mesmo (arquivo .nms do disco).
        if(skipModuleSwitch) return;
  
        if(
          typeof UI !== 'undefined' &&
          UI.switchModule
        ){
          UI.switchModule('config');
        }
        else{
          try{
            CONFIG.init();
  ...
  async loadProjectFromBackend(projectId) {
    ...
    try {
      ...
      const response =
        await fetch(
          APP(`/sistemas/nes/backend/projects/load.php?id=${encodeURIComponent(projectId)}`),
          {
            method: 'GET',
            credentials: 'same-origin'
          }
      ...
      await this.loadFromFile(file, { skipModuleSwitch: true });
