...
const CHAR = (() => {
  ...
  let selectedPhaseIdx = 0; // fase ativa — filtra metatiles pela pág de sprites
  ...
  function getPhases(){
    return (Project.data && Array.isArray(Project.data.phases)) ? Project.data.phases : [];
  }
  /** Página de sprites da fase (NROM=0; CNROM=phase.sprite_page). */
  function phaseSpritePage(phase){
    const mapper = (Project.data && Project.data.mapper === 3) ? 3 : 0;
    if(mapper !== 3) return 0;
    if(!phase) return 0;
    if(phase.sprite_page !== undefined && phase.sprite_page !== null) return phase.sprite_page|0;
    return (phase.bank|0) * 2;
  }
  function getFilteredMetatiles(){
    const all = (typeof CHR !== 'undefined' && CHR.getMetatiles) ? (CHR.getMetatiles()||[]) : (Project.data?.metatiles||[]);
    const phases = getPhases();
    if(!phases.length) return all;
    if(selectedPhaseIdx < 0 || selectedPhaseIdx >= phases.length) selectedPhaseIdx = 0;
    const page = phaseSpritePage(phases[selectedPhaseIdx]);
    return all.filter(m => (m.bank|0) === page);
  }
  function selectPhase(i){
    selectedPhaseIdx = Math.max(0, parseInt(i,10)||0);
    render();
  }
  ...
  const HITBOX_TYPES = ['body','attack','hurt'];

  // Pra uma animacao dada, so' existe UMA hitbox "em vigor" por tipo: a
  // exclusiva dessa animacao (hb.animId===animId), se existir; senao a
  // global (hb.animId vazio). As duas nunca aparecem juntas - impedir isso
  // e' o ponto (evita a global "vazar" pra dentro de uma animacao que tem
  // uma local do mesmo tipo, tipo o corpo-de-baixo global sendo mascarado
  // pela cabeca local no calculo de chao). Retorna so' as que existem.
  function effectiveHitboxesForAnim(c, animId){
    const boxes = ensureHitboxes(c);
    const out = [];
    HITBOX_TYPES.forEach(t=>{
      const local = animId ? boxes.find(h=>h.type===t && h.animId===animId) : null;
      const glob = boxes.find(h=>h.type===t && !h.animId);
      const pick = local || glob;
  ...
  const HITBOX_TYPE_DEFAULT_NAME = { body:'Corpo', attack:'Ataque', hurt:'Dano' };
  function addHitboxOfType(type){
    const c=current(); if(!c)return;
    const a=anim();
    // Ja existe uma hitbox desse tipo em vigor pra animacao atual (global ou
    // local dela)? Nao deixa criar outra - so' confunde e a 2a nunca seria
    // usada mesmo (motor so' considera 1 hitbox por tipo por animacao).
    if(effectiveHitboxesForAnim(c, a?.id).some(hb=>hb.type===type)) return;
    const boxes=ensureHitboxes(c);
    boxes.push({ id:'hb_'+Date.now(), name:HITBOX_TYPE_DEFAULT_NAME[type]||type, type, x:0, y:0, w:8, h:16 });
    render();
  }
  function updateHitboxField(id, field, value){
    ...
    if(field==='name') box.name=value;
      ...
      const a=anim();
      // mesma trava do addHitboxOfType: nao deixa mudar pra um tipo que ja
      // esta em vigor (global ou local) nessa animacao - ficaria 2 iguais.
      const occupied = effectiveHitboxesForAnim(c, a?.id).some(hb=>hb.type===value && hb.id!==box.id);
      if(occupied) { render(); return; }
      box.type=value;
    }
  ...
  function addFrame(){
    ...
    const mts=getFilteredMetatiles();
    if(!mts.length){alert('Nenhum metatile na página de sprites desta fase.\nCrie no CHR Editor ou mude a fase.');return;}
  ...
  function draw(){
    ...
    // Fase 9 (hitbox por animacao): desenha so' a hitbox EM VIGOR de cada
    // tipo pra essa animacao (local se existir, senao a global) - nunca as
    // duas juntas, senao confunde (e no caso do corpo, quem manda no chao
    // e' so' uma delas mesmo).
    (c?effectiveHitboxesForAnim(c, curAnim?.id):[]).forEach(hb=>{
  ...
  function renderEditor(){
    ...
    const phases = getPhases();
    if(phases.length && selectedPhaseIdx >= phases.length) selectedPhaseIdx = 0;
    const mts = getFilteredMetatiles();
    const phaseOpts = phases.length
      ? phases.map((ph,i)=>{
          const sp = phaseSpritePage(ph);
          return `<option value="${i}" ${i===selectedPhaseIdx?'selected':''}>${esc(ph.name||('Fase '+(i+1)))} · SPR pág ${sp}</option>`;
        }).join('')
      : `<option value="0">Sem fases (todos os metatiles)</option>`;
    el.innerHTML=`
      ...
      <div style="font-size:10px;color:#4ec9b0;margin:10px 0 5px">HITBOXES DESTA ANIMAÇÃO <span style="color:#666">(1 de cada tipo)</span></div>
      ${(()=>{
        const eff = effectiveHitboxesForAnim(c, a?.id);
        const effTypes = eff.map(hb=>hb.type);
        return eff.map(hb=>`
        <div style="background:#111;border:1px solid #333;border-radius:5px;padding:6px;margin-bottom:6px">
          <div style="display:flex;gap:5px;align-items:center;margin-bottom:4px">
            ...
            <select onchange="CHAR.updateHitboxField('${hb.id}','type',this.value)" style="background:#000;color:#fff;border:1px solid #444;padding:4px;font-size:10px">
              ${HITBOX_TYPES.map(t=>`<option value="${t}" ${hb.type===t?'selected':''} ${(t!==hb.type && effTypes.includes(t))?'disabled':''}>${t}</option>`).join('')}
            </select>
        ...
        </div>`).join('') || `<div style="font-size:9px;color:#666;margin-bottom:6px">Nenhuma hitbox pra esta animação ainda.</div>`;
      })()}
      <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px">
        ${HITBOX_TYPES.filter(t=>!effectiveHitboxesForAnim(c,a?.id).some(hb=>hb.type===t)).map(t=>
          `<button class="btn-tool" onclick="CHAR.addHitboxOfType('${t}')">＋ ${t}</button>`).join('')}
      </div>
      ...
      <div style="font-size:10px;color:#4ec9b0;margin:8px 0 5px">FASE (filtra metatiles pela pág. de sprites)</div>
      <select id="charPhaseSelect" onchange="CHAR.selectPhase(this.value)" style="width:100%;background:#000;color:#ffcc00;border:1px solid #665500;border-radius:4px;padding:6px;margin-bottom:8px;font-size:12px">
        ${phaseOpts}
      </select>
      <div style="font-size:10px;color:#4ec9b0;margin:4px 0 5px">METATILES DISPONÍVEIS <span style="color:#666;font-weight:normal">(pág sprites da fase)</span></div>
      <div style="max-height:180px;overflow:auto">${mts.map(m=>`<button onclick="CHAR.useMetatile('${m.id}')" style="display:block;width:100%;text-align:left;background:#111;color:#bbb;border:1px solid #333;padding:6px;margin:3px 0;cursor:pointer">${esc(m.name)} <span style="color:#666">(${m.w}x${m.h}) PT${m.bank||0}</span></button>`).join('')||'<span style="color:#666">Nenhum metatile nesta página de sprites. Crie no CHR Editor ou escolha outra fase.</span>'}</div>
  ...
  return {
    init, loadData, render, addCharacter, duplicate, deleteCharacter, selectCharacter, selectPhase, selectAnim, selectFrame,
    addAnim, removeAnim, addFrame, removeFrame, moveFrame, useMetatile, setFrameDuration, setField,
    togglePlay, prevFrame, nextFrame, addHitboxOfType, updateHitboxField, deleteHitbox,
